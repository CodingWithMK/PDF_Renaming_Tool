"""Turkish tokenizer using NLTK with frequency-based word extraction."""
...
class TurkishTokenizer(Tokenizer):
    """Turkish tokenizer that filters stopwords and extracts top words by frequency.

    Turkish doesn't use POS-based noun extraction like English; instead,
    we rely on frequency analysis after stopword filtering.
    """
    ...
    def __init__(self, custom_stopwords: set[str] | None = None) -> None:
        ...
        try:
            self._stopwords = set(stopwords.words("turkish"))
        except LookupError:
            logger.warning("NLTK 'stopwords' not downloaded; using custom only")
        self._stopwords = (
            self._stopwords | CUSTOM_STOPWORDS_TR | (custom_stopwords or set())
        )

    def tokenize(self, text: str) -> list[str]:
        """Tokenize Turkish text and filter stopwords.
